...
#define NUM_BATCHES 9600 //divide the simulations to be completed in batches
#define N_ENSEMBLE 100000 //total number of trajectories per batch
#define N_STEPS 20 // the max amount of steps of each trajectory
...
void get_y2(int x0, int y0, int N, double F, double* randvec, double* y2_final){
	//int N = N_STEPS;
	...
	for (int j = 0; j<N; j++) {
		...
		y_bare = y_bare + current_step.y;
	}
	
	
	for (int sx = x0 - N; sx <= x0 + N; sx++) {
		for (int sy = y0 - N; sy <= y0 + N; sy++) {
	...
	double Dy = 1 / (2 + 2 * cosh(F/2));
	
	
	yf2 = yf2 + (1-p_in) * 2 * Dy * N;
	
	
	yf2 = yf2 - pow(y_bare, 2);
	//printf("%d \n", cnt);
...
void run_sim(int iteration_thread, int seed, double F){
	...
	int traj_id;
	double *y2_samples = (double *)malloc(N_ENSEMBLE * sizeof(double));
	
    double y2_final;
	int N_array[] = {4, 8, 12, 15, 18};
	int N_array_size = 5;
	
	double y2_avg[N_array_size];
	...
	for (int N_ind = 0; N_ind < N_array_size; N_ind++){
		...
		for (traj_id =0; traj_id<N_ENSEMBLE; traj_id++){
			...
			get_y2(x0, y0, N, F, randmat[traj_id], &y2_final);
			...
			y2_samples[traj_id] = y2_final;
			//printf("final pos is (%d, %d) \n", x_samples[traj_id], x2_samples[traj_id]);
			
			
			y2_avg[N_ind] = y2_avg[N_ind] + y2_samples[traj_id];
		}
		////////////////////////////////////////////////////////////
		y2_avg[N_ind] = y2_avg[N_ind] / (double)(N_ENSEMBLE);
		//test area
		//printf("Correction in n: %.8f\n", y2_avg[N_ind] / N );
		//printf("N_STEPS = %d\n", N);
		//end of test area
	}		
	//save all to files
	//we are saving x and x^2 already averaged over ensemble and obstacle realizations
	char filename_y2[100];
	int numElements = N_array_size;
	
	// Define the folder where you want to save the file
	char folder[30];  // Change this to your desired folder
	sprintf(folder, "obs_runs_F_%.3lf/", F);

	// Construct the full path for the file
	sprintf(filename_y2, "%sdata_y2_run_%d_F_%.3lf_N_%d.bin", folder, iteration_thread, F, N_STEPS);
	
	writeDoubleArrayToBinaryFile(y2_avg, numElements, filename_y2);
...
int main(int argc, char* argv[]) {
	...
	double F_array[] = {0.0, 1.0, 2.0, 4.0, 6.0};
	int F_array_size = 5;
	double F;
