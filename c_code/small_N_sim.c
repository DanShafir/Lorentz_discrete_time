...
#define NUM_BATCHES 480 //divide the simulations to be completed in batches
#define N_ENSEMBLE 1000000 //total number of trajectories per batch
...
void run_sim(int iteration_thread, int seed, double F){
	...
	double x_avg[N_STEPS];
	double x2_avg[N_STEPS];
	...
	for (int N = 1; N<=N_STEPS; N++){
		...
		for (traj_id =0; traj_id<N_ENSEMBLE; traj_id++){
			...
			x_avg[N-1] = x_avg[N-1] + x_samples[traj_id];
			x2_avg[N-1] = x2_avg[N-1] + x2_samples[traj_id];
		}
		////////////////////////////////////////////////////////////
		x_avg[N-1] = x_avg[N-1] / (double)(N_ENSEMBLE);
		x2_avg[N-1] = x2_avg[N-1] / (double)(N_ENSEMBLE);
		//test area
...
int main(int argc, char* argv[]) {
	...
	double F_array[] = {0.2, 0.8, 1.0, 1.2, 1.3, 1.4, 1.7, 2.3};
	int F_array_size = 8;
	double F;
